...
from Uni_MoE_audio.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, AUDIO_TOKEN_INDEX, VIDEO_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def get_tokenizer(self):
        tokenizer = AutoTokenizer.from_pretrained("/GLOBALFS/hitsz_bthu_1/jsy/model/llava-lora-merge-audio-4moe-small-v1.5-7b", use_fast=False)
        return tokenizer
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if pretrain_mm_mlp_adapter is not None:
            print("mm projector:", pretrain_mm_mlp_adapter)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def tokenizer(self):
        return self.get_model().get_tokenizer()
    ...
    def encode_audios(self,audios,padding_mask):
        encoder_output = self.get_model().get_audio_tower()(audios,padding_mask)
    ...
    def prepare_inputs_labels_for_audio_and_vision(
        self, input_ids, attention_mask, past_key_values, labels, images, image_mask, videos, video_mask, input_features, features_mask, padding_masks
    ):

        vision_tower = self.get_vision_tower()
        audio_tower = self.get_audio_tower()
        if (vision_tower is None and audio_tower is None) or (images is None and  input_features is None and videos is None) or (input_ids.shape[1] == 1 and input_ids[0][0] == 1):
        ...
        # video:( bs x frame_num x 3 x 336 x 336 ) to ( bs x 576 x 4096 )
        ...
        if input_features is not None:
            ...
            for batch_idx, cur_input_features in enumerate(input_features):
                ...
                cur_padding_mask = padding_masks[batch_idx]
                audio_embeds = []
                tmp_embeds = []
                aucheck = []
                for pidx,cfm_id in enumerate(cur_features_mask):
                    ...
                    pmask = cur_padding_mask[pidx,:,:]
                    audio_embed = self.encode_audios(pfeatures,pmask)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while image_token_indices.numel() > 0 or audio_token_indices.numel() > 0 or video_token_indices.numel() > 0:
                ...
                # if input_ids:[...image...audio...] cut for image
                ...
                # if input_ids:[...video...audio...] cut for video
            ...
            # due input_embeds and labels for each batch 
