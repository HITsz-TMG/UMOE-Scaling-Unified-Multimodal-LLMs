...
def train():
    ...
    if model_args.vision_tower is not None or model_args.audio_tower is not None:
        ...
        cfg_pretrained.num_experts = model_args.num_experts
        cfg_pretrained.num_experts_per_tok = model_args.num_experts_per_tok
        ...
        files = os.listdir(model_args.mlp_dir)
        assert model_args.num_experts == len(files)
        moe_dict = {}
        for i, f in enumerate(files):
            state_dict = torch.load(os.path.join(model_args.mlp_dir, f),torch.device('cpu'))
            print("relative mlp:",i,f)
            for key,value in state_dict.items():
                # need change
                name = "model.layers."+key.split(".")[2]+".mlp.experts."+str(i)+"."+key.split(".")[4]+".weight"
                moe_dict[name] = value
    ...
    # train_module = ["gate_proj", "up_proj", "down_proj", "gate", "up", "down"]
    train_module = ["gate_proj"]
