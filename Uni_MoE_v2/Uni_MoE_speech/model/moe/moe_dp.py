...
def deepspeed_forward(self):
    def forward(
        ...
        return output, self.deepspeed_moe.l_aux, self.deepspeed_moe.logits, self.deepspeed_moe.exp_counts
    return forward

def deepspeed_moe_layer_forward(self):
    def forward(*input: Tensor, **kwargs: Any) -> Tensor:
        ...
        if self.use_tutel:
            self.l_aux, C, E, indices_, locations_, gates_, self.exp_counts, self.logits = self.gate(reshaped_input, input[1], True)
            ...
            self.l_aux, combine_weights, dispatch_mask, self.exp_counts, self.logits = self.gate(reshaped_input, input[1])
...
def deepspeed_moe_gate_forward(self):
    def forward(
        input: torch.Tensor,
...
class MoEFm(MoE):
    ...
    def __init__(self,
        ...
        # super(MoE, self).__init__(hidden_size, expert, layer_num, num_experts, ep_size, k, capacity_factor,
                                #   eval_capacity_factor, min_capacity, use_residual, noisy_gate_policy, drop_tokens,
                                #   use_rts, use_tutel, enable_expert_tensor_parallelism, top2_2nd_expert_sampling)
        ...
        self.deepspeed_moe = MOEFmLayer(TopKGate(hidden_size, num_experts, k, capacity_factor, eval_capacity_factor,
    ...
    def forward(self,
                hidden_states: torch.Tensor,
                return_indice: Optional[dict] = None,
                used_token: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """ MoE forward
        ...
        """
        output = self.deepspeed_moe(hidden_states, used_token, return_indice)
        if self.use_residual:
            # Residual MoE
            output_mlp = self.mlp(hidden_states)
            if isinstance(output_mlp, tuple):
...
class MOEFmLayer(MOELayer):
    ...
    def __init__(self,
        ...
        self.use_fm_block = True
        self.hidden_size = 4096
        
        if self.use_fm_block:
            self.fm_audio = nn.Conv1d(in_channels=self.hidden_size, out_channels=self.hidden_size, kernel_size=1, bias=True)
            self.fm_video= nn.Conv1d(in_channels=self.hidden_size, out_channels=self.hidden_size, kernel_size=1, bias=True)
            self.fm_image = nn.Conv1d(in_channels=self.hidden_size, out_channels=self.hidden_size, kernel_size=1, bias=True)
            self.fm_text = nn.Conv1d(in_channels=self.hidden_size, out_channels=self.hidden_size, kernel_size=1, bias=True)
    ...
    def forward(self, *input, **kwargs: Any) -> Tensor:
        ...
        # group_size = kwargs['group_size'] if 'group_size' in kwargs.keys() else 1
        reshaped_input = input[0].reshape(-1, d_model)
        return_indice = input[2]
        
        if self.use_fm_block:
            # print("no fm!")
            gate_input = input[0].clone()
            for batch_idx in range(input[0].shape[0]):
                if return_indice is not None:
                    ...
                    # print("indices:\n",image_indice, audio_indice, video_indice, text_indice)

                    if len(image_indice) != 0:
                        for img_ind in image_indice:
                            gate_input[batch_idx][img_ind[0]:img_ind[1]] = self.fm_image(gate_input[batch_idx][img_ind[0]:img_ind[1]].clone().unsqueeze(0).permute(0,2,1)).permute(0,2,1)
                    if len(audio_indice) != 0:
                        for aud_ind in audio_indice:
                            gate_input[batch_idx][aud_ind[0]:aud_ind[1]] = self.fm_audio(gate_input[batch_idx][aud_ind[0]:aud_ind[1]].clone().unsqueeze(0).permute(0,2,1)).permute(0,2,1)
                    if len(video_indice) != 0:
                        for vid_ind in video_indice:
                            gate_input[batch_idx][vid_ind[0]:vid_ind[1]] = self.fm_video(gate_input[batch_idx][vid_ind[0]:vid_ind[1]].clone().unsqueeze(0).permute(0,2,1)).permute(0,2,1)
                    if len(text_indice) != 0:
                        for text_ind in text_indice:
                            gate_input[batch_idx][text_ind[0]:text_ind[1]] = self.fm_text(gate_input[batch_idx][text_ind[0]:text_ind[1]].clone().unsqueeze(0).permute(0,2,1)).permute(0,2,1)
                else:
                    gate_input[batch_idx] = self.fm_text(gate_input[batch_idx].clone().unsqueeze(0).permute(0,2,1)).permute(0,2,1)

            reshaped_gate_input = gate_input.reshape(-1, d_model)

            assert reshaped_gate_input.shape == reshaped_input.shape, f"reshaped_gate_input shape: {reshaped_gate_input.shape} != reshaped_input shape: {reshaped_input.shape}"


        if self.use_tutel:
            self.l_aux, C, E, indices_, locations_, gates_, self.exp_counts, self.logits = self.gate(reshaped_gate_input, input[1], True)
            S, M = reshaped_input.size(0), reshaped_input.size(1)
            ...
            self.l_aux, combine_weights, dispatch_mask, self.exp_counts, self.logits = self.gate(reshaped_gate_input, input[1])
            dispatched_input = einsum("sec,sm->ecm", dispatch_mask.type_as(input[0]), reshaped_input)
